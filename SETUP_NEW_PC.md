...
### Проблема: Помилка при імпорті дампу бази даних
**Рішення:**
```powershell
# Якщо помилка "database already exists"
psql -U postgres
DROP DATABASE lms_db;
CREATE DATABASE lms_db;
\q

# Якщо помилка з правами доступу
# Переконайтеся що використовуєте правильного користувача
$env:PGPASSWORD = "ваш_пароль"
pg_restore -U postgres -h localhost -d lms_db -v "backup_lms_db.dump"
```

### Проблема: Версії PostgreSQL не співпадають між ПК
**Рішення:**
- Встановіть ту саму версію PostgreSQL на новому ПК
- Або експортуйте в SQL формат (`pg_dump -F p`) замість Custom формату
- SQL формат більш сумісний між версіями

### Проблема: Помилка з міграціями Prisma після імпорту
**Рішення:**
```powershell
# Якщо міграції вже застосовані в імпортованій базі
# Позначте їх як застосовані:
npx prisma migrate resolve --applied 20251029092810_init
npx prisma migrate resolve --applied 20251030083324_add_requested_course_id
npx prisma migrate resolve --applied 20251030085941_add_lesson_scheduled_at
```
